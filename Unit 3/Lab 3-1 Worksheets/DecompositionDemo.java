public class DecompositionDemo {
    ...
    public static void bigMethodToDecompose() {
        ...
        double totalSteps = getTotalSteps(steps, hours);
        double avgSteps = getAvgSteps(steps, hours);
...
public static double[] calcAverage(int[] steps) {
    // Morning vs Midday vs Afternoon averages
    // Morning: 0-3, Midday: 4-7, Afternoon: 8-11
    ...
    double[] averages = { avgMorning, avgMidday, avgAfternoon };
    return averages;
}

public double activeMinutesSummary(int[] activeMinutes, int[] hours) {
    int n = hours.length;
    int totalActiveMinutes = 0;
    int activeHourCount = 0; // >= 30 minutes counts as active hour
    for (int i = 0; i < n; i++) {
...
public double getTotalSteps(int[] steps, int[] hours) {
    int n = hours.length;
    int totalSteps = 0;
...
public double getAvgSteps(int[] steps, int[] hours) {
    double totalSteps = getTotalSteps(steps, hours);
    double avgSteps = totalSteps / (double) hours.length;
    return avgSteps;
}
